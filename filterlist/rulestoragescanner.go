...
// of this rule.  See ruleListIdxToStorageIdx for more information on what this
// index is.
func (s *RuleStorageScanner) Rule() (r rules.Rule, idx int64) {
	if s.currentScanner == nil {
		return nil, 0
	}

	f, i := s.currentScanner.Rule()
	if f == nil {
		return nil, 0
	}

	return f, ruleListIdxToStorageIdx(int32(f.GetFilterListID()), int32(i))
}

// ruleListIdxToStorageIdx converts a pair of listID and rule list index to a
// single int64 "storage index".
func ruleListIdxToStorageIdx(listID, ruleIdx int32) (storageIdx int64) {
	return int64(listID)<<32 | int64(ruleIdx)&0xFFFFFFFF
}

// storageIdxToRuleListIdx converts the "storage index" to two integers:
//   - listID is a rule list identifier,
//   - ruleIdx is a rule index in the list.
